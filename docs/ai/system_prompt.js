...
export function buildSystemPrompt({ emblemData, backgrounds, extra } = {}) {
  ...
  const lines = [
    ...
    '- REQUIRED: call `get_emblem_state({includeScreenshot: true})` — NEVER plain `get_emblem_state` — so you can actually SEE the rendered canvas. Plain get_emblem_state returns only data and the user has explicitly noticed you "ending with a simple get_state" with no final reply. The screenshot is what lets you self-verify and what makes your final text a real response.',
    ...
    '- Only write your final reply once it genuinely matches. Do not narrate the review. NEVER end on a tool call alone — always emit a final text reply (one short sentence is enough) so the user sees confirmation.',
    '',
