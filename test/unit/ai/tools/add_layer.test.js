...
describe('add_layer', () => {
  ...
  it('calls editor.changestacki then repaints via generatestackcanvas + createfilter (order matters for visual selection)', async () => {
    const c = ctx();
    c.editor.generatestackcanvas = vi.fn();
    c.editor.createfilter = vi.fn();
    c.editor.changestacki = vi.fn();
    await execTool('add_layer', { name: 'Letter A', position: 5, hue: 0.3, saturation: 0.7 }, c);
    // changestacki sets stacki AND updates the .selected CSS class on the
    // layer preview — without this, the visual selection drifts and the
    // user can end up with multiple layers highlighted after multiple ops.
    expect(c.editor.changestacki).toHaveBeenCalledOnce();
    expect(c.editor.changestacki).toHaveBeenCalledWith(4);
    ...
    expect([h, s, v, a]).toEqual([0.3, 0.7, 1, 1]);
    // createfilter called BEFORE generatestackcanvas so the re-paint bakes
    // the new filter into stack[i].canvas (used by the main canvas).
